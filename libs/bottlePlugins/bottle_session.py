...
# Only two changes. Everithing else is as created by Christopher De Vries
# 1. Comented un used redis import
# 2. Changed the string for session id from "session:" to "session_"
# Add if else for ram_db and import of module os
...
class Session(object):
    ...
    def validate_session_id(self,cookie_value):
        try:
            ...
            keycheck = 'session_%s'%str(uuid.UUID(cookie_value))
        except:
            self.new_session_id()
            return
